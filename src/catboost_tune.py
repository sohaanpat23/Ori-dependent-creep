"""
CatBoost baseline using the exact same split logic as the PINN and other baselines.

Logic:
- Load raw processed dataset.
- Engineer same 11 features.
...
- Tune CatBoost using GridSearchCV + PredefinedSplit.
- Generate OOF predictions on Train/CV pool.
- Evaluate 20 unseen internal validation points using 10 fold-model ensemble.

Feature logic:
- T_C is NOT used directly because Inv_T_K is already present.
- stress_MPa is included, giving 11 total features.
"""
...
def get_catboost_model_and_grid() -> Tuple[Pipeline, Dict]:
    ...
    # Practical grid, similar in scale to the other baselines.
    # bootstrap_type="Bernoulli" is included so that subsample is valid.
    param_grid = {
        ...
        "cat__subsample": [0.8, 1.0],
        "cat__random_strength": [0.5, 1.0, 2.0],
    }
...
def run_catboost_baseline(
    ...
    pipe, param_grid = get_catboost_model_and_grid()
    ...
    print(f"\nBest CATBOOST CV score, neg MAE: {search.best_score_}")
    ...
    if np.isnan(oof_pred).any():
        raise RuntimeError("CATBOOST: Some OOF predictions are NaN.")

    oof_df = train_cv_df.copy()

    actual_col = "CATBOOST_Actual_Log10_t_rupture_h"
    pred_col = "CATBOOST_Pred_Log10_t_rupture_h"
    pred_h_col = "CATBOOST_Pred_t_rupture_h"
    ape_col = "CATBOOST_APE_%"
    ...
    h_actual_col = "CATBOOST_Actual_Log10_t_rupture_h"
    h_pred_col = "CATBOOST_Pred_Log10_t_rupture_h"
    h_std_col = "CATBOOST_Pred_Log10_Ensemble_STD"
    h_pred_h_col = "CATBOOST_Pred_t_rupture_h"
    h_ape_col = "CATBOOST_APE_%"
    ...
    print("\n================ CATBOOST 20-POINT INTERNAL VALIDATION ================")
    ...
    # Saved only for reference. Main holdout metric uses fold ensemble.
    ...
    holdout_result_df[
        "CATBOOST_FullTrainCV_Pred_Log10_t_rupture_h"
    ] = full_pred

    holdout_result_df[
        "CATBOOST_FullTrainCV_Pred_t_rupture_h"
    ] = 10 ** full_pred

    holdout_result_df[
        "CATBOOST_FullTrainCV_APE_%"
    ] = (
        np.abs(
            holdout_result_df[
                "CATBOOST_FullTrainCV_Pred_t_rupture_h"
    ...
    oof_path = model_dir / "catboost_oof_preds_train_cv.csv"
    fold_metrics_path = model_dir / "catboost_fold_metrics.csv"
    zone_metrics_path = model_dir / "catboost_zone_metrics.csv"
    holdout_path = model_dir / "catboost_internal_validation_20_unseen.csv"
    holdout_metrics_path = model_dir / "catboost_internal_validation_metrics.csv"
    best_params_path = model_dir / "catboost_best_params.txt"
...
def main() -> None:
    ...
    output_dir = Path(CONFIG.output_root) / f"catboost_same_split_run_{run_stamp}"
    output_dir.mkdir(parents=True, exist_ok=True)

    split_audit_path = output_dir / "catboost_split_assignments.csv"
    train_cv_path = output_dir / "catboost_train_cv_with_folds.csv"
    holdout_path = output_dir / "catboost_internal_validation_20_raw.csv"
    ...
    summary_path = output_dir / "catboost_summary_metrics.csv"
